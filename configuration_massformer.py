...
MASSFORMER_PRETRAINED_CONFIG_ARCHIVE_MAP = {
    "clefourrier/graphormer-base-pcqm4mv2": "https://huggingface.co/clefourrier/graphormer-base-pcqm4mv2/resolve/main/config.json",
}



class MassFormerConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`~MassFormerModel`]. It is used to instantiate an
    MassFormer model according to the specified arguments, defining the model architecture. Instantiating a
    configuration with the defaults will yield a similar configuration to that of the MassFormer
    [clefourrier/graphormer-base-pcqm4mv2](https://huggingface.co/clefourrier/graphormer-base-pcqm4mv2) architecture.
    ...
    Args:
        ...
        pre_layernorm (`bool`, *optional*, defaults to `False`):
            ...
            used.
        apply_massformer_init (`bool`, *optional*, defaults to `False`):
            Apply a custom massformer initialisation to the model before training.
        ...
        Example:
            ```python
            >>> from transformers import MassFormerForGraphClassification, MassFormerConfig

            >>> # Initializing a MassFormer massformer-base-pcqm4mv2 style configuration
            >>> configuration = MassFormerConfig()

            >>> # Initializing a model from the clefourrier/graphormer-base-pcqm4mv2 style configuration
            >>> model = MassFormerForGraphClassification(configuration)
